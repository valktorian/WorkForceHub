...
public class ReopenTimesheetHandler : ICommandHandler<ReopenTimesheetCommand, CommandAcceptedResponse>
{
    ...
    public ReopenTimesheetHandler(ITimeCommandRepository<Timesheet> repository, IUnitOfWork unitOfWork)
    ...
    public async Task<CommandAcceptedResponse> HandleAsync(ReopenTimesheetCommand command, CancellationToken cancellationToken = default)
    {
        ...
        entity.Reopen(command.Comment);
        var evt = TimeEventFactory.Status(entity);
        await _repository.UpdateAsync(entity, evt, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
        return new CommandAcceptedResponse(entity.Id, entity.Status, "Timesheet reopened.", evt);
    }
}
